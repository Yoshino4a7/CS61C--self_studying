...
mapLoop:
    ...
    lw t4, 0(s0)     # load the address of the array of current node into t1 t1寄存器已经被使用，应当用t4寄存器
    lw t2, 4(s0)        # load the size of the node's array into t2
    
   
	slli t3,t0,2     #将conut*4之后的值赋给t3寄存器，作为数组一次循环偏移的位移量
	add t4,t4,t3	#使数组的元素指针偏移
    ...
    lw a0, 8(s0)        # load the address of the next node into a0 在结构中读取数据应当用lw
    #不需要再在s1读取f函数的地址
